...
<head>
<meta charset="UTF-8">
<title>03InnerObject > ApplicationMain.jsp</title>
...
<body>
	...
	입력해야 한다.
	http://localhost:9999/K07JSPServlet/03InnerObject/ResponseMain.jsp
	localhost : 
	K07JSPServlet : 프로젝트
	 --%>
	...
	<%--
	선언부(자바영역)에서 JSP의 내장객체를 사용할때는 해당 내장객체의
	클래스타입으로 사용해야 한다. application객체는
	ServletContext타입을 가지고 있다.
	 --%>
	<h2>선언부에서 물리적경로 사용하기</h2> <!-- 선언부 많이 안씀! 보통 자바영역에 클래스함수 정의 -->
	<%! //! 붙는 선언부는 자바의영역이라고 생각하세욤. 많이안씀...
	//1.this 키워드 사용하기
	String getRealPath(){
		return
	...
	반드시 매개변수 형태로 전달한후 내장객체의 클래스형으로 받은 다음
	사용해야 한다.
	*/
	...
	<%//이거 지우라네염...
	//JSP(WebContent)
	out.println("하하하하"); //JSP영역이니 출력가능
	//static이 아니니 객체생성하기
	FirstFunction ff = new FirstFunction();
	ff.tempFunc(out); //out내장객체 전달
	%>
