...
<body>
	<fieldset>
		...
		<!-- 
			...
			연산된다. 숫자로 변경할수 없는 문자가 포함된다면 예외가 발생된다.
			또한 문자열끼리 연결도 EL에서는 허용되지 않는다.
		 -->
		...
		<ul>
			...
			<li>${"HELLO" } ${"EL"} <!-- 문자열은 이와같이 표현하면 됨 --></li>
		</ul>
		...
		<ul>
			<li>존재하지않는 속성 : \${eldto.addr }
				:
				\${eldto.addr } [PropertyNotFoundException발생]
			</li>
			<!-- MemberDTO에서 getId 주석처리 후 테스트 -->
		...
		<ul>
			<li>\${reqstScopr.reqProperty } :
					${reqstScopr.reqProperty } [정상]</li>
			<li>\${reqstScopr.REQProperty } :
					${reqstScopr.REQProperty } [에러NO, 미출력]</li>
