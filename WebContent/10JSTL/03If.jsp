...
<body>
	...
	-else문이 없기때문에 다중조건일때는 표현이 어렵다. 이때는
	주로 choose문을 사용한다.
	형식]
		test : 조건
	...
	<!-- 
		...
		var 속성에 지정된 result에 true가 저장된다.
	 -->
	...
	<!-- 
		JSTL의 if문은 else가 없기때문에 참or거짓의 형태로
	...
	<c:if test="${not empty param.user }">
		...
		<!-- 
		첫번째 if문에서의 결과에 따라 두번째 if문의 반대의 결과를
		가지게되는 구조롤 작성되었다. 즉 loginResult가 true가
		된다면 두번째 if문은 false값으로 바뀌게 된다.
		 -->
		<c:if test="${param.user eq 'kosmo' and param.pass==1234}"
			var="loginResult">
			${param.user }님, 하이룽~!!
		</c:if>
		<!-- 아니면 위의 조건자체에 !(not)으로 해도됨. !조건은 내가 붙인거-->
		<c:if test="${not loginResult && !loginResult==''}">
			아이디와 비번을 확인하세요.
		</c:if>
	</c:if>
