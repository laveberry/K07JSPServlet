...
<body>
	...
	set태그속성
		var : 변수명으로 문자열만 가능함.
		value : 값을 설정함. 표현식, EL식 모두 가능함.
		scope : 영역을 지정하여 변수를 저장함.
			4가지 영역을 지정할수 있고 기본값은 가장좁은
			page영역.
		즉, set태그를 사용하여 변수를 저장하면 지정된 영역에
		저장된다. 		
	-->
	<h2>set태그</h2>
	...
	<h3>EL로 출력</h3>
 	<!--  
 		...
 		영역인 page영역을 읽어오게되므로, 속성명이 동일하지 않다면
 		내장객체는 생략할수 있다. 
 	-->
	<ul>
	...
	<h3>인자생성자로 설정</h3>
	<!-- 인자생성자를 통해 DTO객체를 생성한후 request영역에 속성을 저장한다. -->
	<c:set var="argsMember"
		value='<%=new MemberDTO("KIM","1111","김태희",null) %>'
		scope="request"	/>
	
	<h3>JSP코드로 출력</h3>
	<!-- 영역에 저장될때 Object타입으로 자동형변환 되므로 사용시
	...
	<!--  
		...
		않는다면 xxScope 계열의 내장객체를 생략할 수 있다. 
	-->
	<h3>EL로 출력</h3>
	<ul>
	...
	<c:set target="${map.args }" property="id" value="스타트업" />
	<c:set target="${map.args }" property="pass" value="7777" />
	<c:set target="${map.args }" property="name" value="한지평짱" />
