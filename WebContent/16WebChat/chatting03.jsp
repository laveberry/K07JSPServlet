...
// 		var webSocket = new WebSocket('ws://localhost:포트/컨택스트루트/ChatServer02');
// 		var webSocket = new WebSocket('ws://IP주소/컨택스트루트/ChatServer02');
		var webSocket = new WebSocket('ws://localhost:9999/K07JSPServlet/ChatServer02');
		//IP주소 얻기 -> ctrl+R -> cmd -> ipconfig
		
		
		/*
			웹소켓 서버에 연결한 후 메세지 전송, 에러발생 등은 모두 이벤트를
		...
		*/
		...
		function wsMessage(event) {
			...
			if(content == ""){
				...
				if(content.match("/")){
					/* 
						♥전송자체는 모든사용자에게 일단 뿌려지지만 /(명령어)+chat_id(받을사람) 
						확인한 후 받을사람에게만 출력이 되는것이다.
					*/
