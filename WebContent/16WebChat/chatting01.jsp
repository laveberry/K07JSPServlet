...
<body>
	<fieldset style="width: 350px; text-align: center;">
		...
		<input type="text" id="inputMessage" style="width:300px; margin-bottom: 5px;"/>
			...
			<br />
			<!-- 대화출력창 -->
			<!-- 출력창 : textarea는 value값을 이용해 값입력. div등은 innerHTML이용 -->
			<textarea id="messageWindow" style="width:300px; height:400px; margin-top:10px;" readonly></textarea>
	</fieldset>
	
	
	<!-- head에 안적고 여기에 script 적으면
		 이벤트처리(window.onload같은) 덜해도되서 조금더 편함 -->
	<script>
	/*
	...
	*/
		...
		function sendMessage(){
			...
			//웹소켓 서버로 입력한 내용을 전송한다.(send메소드 이용)
			/*
			send() 메소드 : 연결된 서버나 클라이언트로 데이터 전송
			send();       // GET 방식
			send(문자열); // POST 방식
			*/
