...
//페이지처리 문자, 이미지, 부트스트랩 3가지 버전으로 구분!
public class FileUtil {
	...
	public static void download(
		...
		try{
			...
			1) 파일명을 byte형 배열로 변환
			2) String클래스의 생성자에 
				변환한 배열과 charset(HTML 문서의 문자 인코딩 방식)는 8859_1을 지정함.
				 */
				sfileName 
			...
			받을때 아래 설정된 응답헤더를 통해 파일명이 원본파일명으로 변경되어
			로컬로 다운로드 된다.
			 */
			response.setHeader("Content-Disposition"
					, "attachment;filename="+ofileName);//여기만 ofileName
